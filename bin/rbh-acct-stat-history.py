...
def take_acct_stat_snapshot( cur, rbh_db, rbh_acct_table, history_db, history_acct_table, date ):
   ...
   sql = "INSERT INTO " + history_db + "." + history_acct_table + " SELECT uid, gid, SUM(size), SUM(count), '" + date + "' FROM " + rbh_db + "." + rbh_acct_table + " WHERE type='file' GROUP BY 1,2"
   
   logging.debug( sql )
   cur.execute( sql )
   
   if not cur.rowcount:
      raise RuntimeError( "Failed to take a snapshot for table: " + history_acct_table + " with date: " + date )
   
   logging.info( "Inserted rows: " + str( cur.rowcount ) + " into table: " + history_acct_table + " for date: " + date )
...
def main():
   ...
   try:
      ...
      with closing( MySQLdb.connect( host=config.get( 'mysqld', 'host' ), user=config.get( 'mysqld', 'user' ), passwd=config.get( 'mysqld', 'password' ), db=rbh_db ) )  as conn:
         with closing( conn.cursor() ) as cur:
            ...
            take_acct_stat_snapshot( cur, rbh_db, rbh_acct_table, history_db, history_acct_table, str ( date_today ) )
